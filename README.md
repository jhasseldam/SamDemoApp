...
```bash
SamDemoApp$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
SamDemoApp$ sam local start-api
SamDemoApp$ curl http://localhost:3000/
```
...
```bash
SamDemoApp$ sam logs -n HelloWorldFunction --stack-name SamDemoApp --tail
```
...
```bash
SamDemoApp$ dotnet test test/HelloWorld.Test
```
...
```bash
sam delete --stack-name SamDemoApp
```
